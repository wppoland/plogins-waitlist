...
// + `restock/booted`, and type-hints `\Restock\Plugin` on the listener it attaches).
// Safe to remove once all PRO installs are >= 1.0.3.
//
// This has to sit below the autoloader. class_alias() loads the class it is
// aliasing, so up at the top of the file, before any autoloader is registered,
// it warned `Class "Waitlist\Plugin" not found` on every single request and
// returned false, which left the alias the old PRO needs undefined anyway.
